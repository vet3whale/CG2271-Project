...
static void build_packet(uint8_t tap, uint8_t on_off, uint8_t paused,
		uint16_t light, uint16_t sound, uint8_t triggered, uint8_t env_cond,
		uint8_t temp, uint8_t temp_frac, uint8_t hum, uint8_t hum_frac,
		uint8_t *pkt) {
	uint8_t chk = 0;
	pkt[0] = PACKET_START1;
	pkt[1] = PACKET_START2;
...
void vTxTask(void *pvParameters) {
	...
	uint8_t tap = 0, on_off = 0, paused = 0, triggered = 0, env_cond =
			ENV_UNKNOWN, temp = 0, temp_frac = 0, hum = 0, hum_frac = 0;
	...
	while (1) {
		...
		if (xSemaphoreTake(gSensorMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
			...
			light = gSensorData.light; //change this backkkkk
			sound = gSensorData.sound; //change this backkkkk
			...
			if (tap)
				gAverageSensorData.tap_event = 0;
