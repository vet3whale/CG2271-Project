...
void led_init(void)
{
    ...
    BLUE_TPM->CONTROLS[BLUE_TPM_CHANNEL].CnV    = 0;
}
...
void vLEDTask(void *pvParameters)
{
    ...
    while (1)
    {
        if (led_state) {
            ...
            if (current_r < target_r) current_r = (current_r + step > target_r) ? target_r : current_r + step;
            else if (current_r > target_r) current_r = (current_r < step) ? 0 : current_r - step;

            if (current_g < target_g) current_g = (current_g + step > target_g) ? target_g : current_g + step;
            else if (current_g > target_g) current_g = (current_g < step) ? 0 : current_g - step;
