...
static void parseAndStore(uint8_t *pkt) {
  ...
  if (on_off) {
    ...
    Serial.println(tap);
    Serial.print("  on_off_mode:     ");
    Serial.println(on_off);
    Serial.print("  paused:          ");
    Serial.println(paused);
    Serial.print("  avg. light_raw:  ");
    Serial.println(light);
    Serial.print("  avg sound_raw:   ");
    Serial.println(sound);
    Serial.print("  sound_triggered: ");
    Serial.println(triggered);
    Serial.print("  env_cond:        ");
    Serial.println(envConditionStr(env_cond));
...
void vUartRxTask(void *pvParameters) {
  ...
  while (1) {
    int received = uart_read_bytes(MCXC_UART_PORT, &byte, 1, pdMS_TO_TICKS(10));
    if (received <= 0) {
      continue;
    }
    ...
    if (idx == PACKET_LEN) {
      ...
      if (buf[PACKET_LEN - 1] != PACKET_END) continue;
      if (!validateChecksum(buf)) continue;
      parseAndStore(buf);
    }
